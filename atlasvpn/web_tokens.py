...
def _jwt_secret() -> str:
    j = (os.environ.get("ATLASVPN_JWT_SECRET") or "").strip()
    if j:
        return j
    from atlasvpn.web_auth import get_session_secret

    return get_session_secret()


def jwt_ttl_seconds() -> int:
    return int(os.environ.get("ATLASVPN_JWT_EXPIRE_SECONDS", "604800"))
